...
.proc do_print_obj
    ; Find the base address of our object
    sta dpo_window ; Save window to print to
    stx dpo_maxlen ; Save max length to print
    ldx operand_0
    ldy operand_0+1
    jsr find_object
    pushb zpu_mem+2
    ...
    lda dpo_window ; Get window to print to
    ldx dpo_maxlen ; Get max length to print
    jsr print_encoded
    popb
    rts
.endproc
...
; do_print_obj temps (moved from $400-$401 to avoid UniLib BSS conflict)
dpo_window: .res 1
dpo_maxlen: .res 1
